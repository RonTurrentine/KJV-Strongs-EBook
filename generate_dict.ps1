...
function Expand-POS([string]$code) {
    if (-not $code) { return '' }
    $lower = $code.ToLower().Trim()
    if ($posMap.ContainsKey($lower)) { return $posMap[$lower] }
    return $code
}
...
function Write-DictPage {
    ...
    $title    = HtmlEscape "$StrongsId — $OriginalWord"
    ...
    $kjvBlock = if ($kjvHtml) { @"
    <div class="strongs-kjv">
      <p class="note-title"><strong>Strong's Definition / KJV Usage</strong></p>
      <p>$kjvHtml</p>
    </div>
"@ } else { '' }
...
<header class="page-header">
  ...
  <p class="chapter-nav">
    ...
    <a href="$idxPath" class="btn" title="Browse $Language index">$Language Index</a>
  </p>
</header>
...
foreach ($entry in $entries) {
    ...
    # Use lemma attribute for fully vowel-pointed Hebrew; fall back to InnerText
